...
本页对应 `0.2.0-preview.1`，采用 PolyForm Noncommercial 1.0.0。源码交付、特定实例升级和通用生产认证分别记录；本版证据见 [UPDATE-VALIDATION](UPDATE-VALIDATION.md)。

## 1. 本版源码预览已核实的内容

- [x] 正式 LICENSE、项目 NOTICE、商业申请、贡献、安全及隐私说明随源码保留。
- [x] 基于公开 preview.5 增量取用 L37-source-r2 的 10 个精确源码/测试文件；保留公开跨 Python AST 测试、组件文档与代理修复。私人部署、配置、数据库和聊天附件不公开。
...
- [x] 本版合入 RikkaHub 已验收的短期换窗、来源识别及辅助通道隔离；Orbis 仍待适配。既有网络和缓存体验反馈保留原始范围，不当作本次新验收。
