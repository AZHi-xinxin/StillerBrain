...
- [x] 从 L36 来源同步 206 份源码、Schema 与测试文件，另保留公开仓库的跨 Python AST 静态测试；组件文档按本版公开用法更新。2026-09-18 获得真实场景验收反馈后补入此前暂缓的内部代理差异及独立回归；私人网络配置不公开，不将整个部署目录原样公开。
...
- [x] 本版合入使用者已验收的核心记忆与 MCP 更新；学校流量/长上下文顺畅作为使用者反馈单列，不冒充全网络性能认证，长期缓存自用反馈仍按原有定性范围描述。
